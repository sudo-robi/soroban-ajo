...
export default function WalletConnectionTest() {
  ...
  useEffect(() => {
    const checkFreighter = async () => {
      setChecking(true)
      const api = await waitForFreighterApi({ timeoutMs: 3000, intervalMs: 100 })
      setFreighterInstalled(!!api)
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      await ensureFreighterAllowed(api)
          ...
          {checking ? (
            ...
            <>
              <div className="space-y-4 mb-8">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-3 h-3 rounded-full ${freighterInstalled ? 'bg-green-500' : 'bg-red-500'}`}
                  />
                ...
                {freighterInstalled && (
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-3 h-3 rounded-full ${freighterAllowed ? 'bg-green-500' : 'bg-yellow-500'}`}
                    />
                    <span className="text-gray-700">
